        ...
        function startSystem() {
            ...
            // Bắt đầu nhạc, có thể thất bại nếu không có tương tác người dùng
            ...
            hands.onResults(results => {
                ...
                // Vẽ ảnh camera (nếu có) lên canvas preview
                if (results.image) {
                ...
                if(results.multiHandLandmarks.length > 0) {
                    ...
                    handX = lm[9].x; // Lấy vị trí ngón giữa (thử nghiệm)

                    const tips = [8,12,16,20]; // Đầu ngón trỏ, giữa, áp út, út
                    const wrist = lm[0];
                    let openDist = 0;
                    tips.forEach(i => openDist += Math.hypot(lm[i].x-wrist.x, lm[i].y-wrist.y));
                    const avgDist = openDist / 4;
                    const pinchDist = Math.hypot(lm[4].x-lm[8].x, lm[4].y-lm[8].y); // Ngón cái và ngón trỏ
                    ...
                    state = 'TREE'; 
                    statusDiv.innerText = "🎄 P.Anh Giáng Sinh VV 🎄"; statusDiv.style.color = "#FFF";
                }
            });
            ...
            const cameraUtils = new Camera(video, {
                onFrame: async () => {
                    frameCnt++; 
                    // Chỉ xử lý 1/3 số frame để giảm tải
            ...
            try {
                ...
                logError("Lỗi truy cập camera: " + e.message);
                statusDiv.innerText = "❌ LỖI: Không truy cập được camera.";
